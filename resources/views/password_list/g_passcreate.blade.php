...
<form method="POST" action="g_store">
            @csrf
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __("管理するパスワードの追加が出来ます。") }}
                ...
                <div class="mt-4">
                <x-input-label for="group" :value="__('グループ選択')" />
                    <select name="group_id" id="group_id">
                        <?php foreach($team_list as $teams){ 
                            $name = $team::select('group_name')->where('group_id','=',$teams->group_id)->first();
                            ?>
                            <option id=<?php $teams->group_id?> name=<?php $teams->group_id?> value="{{ $teams->group_id }}">{{ $name->group_name}}</option>
                            <?php } ?>
